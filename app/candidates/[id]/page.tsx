...
export default function CandidateDetailPage() {
  ...
  if (!data) return <div className="app-page space-y-4"><PageIntro title="Candidate unavailable" /><ErrorAlert message={error || "Candidate not found"} /><Link className="button-secondary" href="/candidates">Back to candidates</Link></div>;
  const { candidate, components, evidence, scoreSignals, activity } = data;
    ...
    <div className="app-page space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
        <div className="space-y-6">
          ...
          <section className="panel p-5"><div className="flex items-start justify-between gap-4"><div><p className="eyebrow">Recorded facts</p><h3 className="panel-title">Public facts used by local rules</h3></div><Badge tone={candidate.facts ? "teal" : "slate"}>{candidate.facts ? "Facts rules" : "Legacy score input"}</Badge></div>{candidate.facts ? <div className="mt-4 grid gap-3 sm:grid-cols-2"><Fact label="Owns audience" value={candidate.facts.ownedAudience ? "Yes" : "No"} /><Fact label="Platforms / frequency" value={`${candidate.facts.activePlatformCount} · ${candidate.facts.contentFrequency}`} /><Fact label="Business signals" value={candidate.facts.course || candidate.facts.coaching || candidate.facts.membership || candidate.facts.ecommerce || candidate.facts.affiliate || candidate.facts.brandDeal ? "Present" : "None recorded"} /><Fact label="Fan relationship" value={[candidate.facts.newsletter, candidate.facts.community, candidate.facts.paidCommunity, candidate.facts.membership].filter(Boolean).length + " signal(s)"} /><Fact label="AI signals" value={[candidate.facts.aiUsage, candidate.facts.aiNative, candidate.facts.virtualCreator].filter(Boolean).length + " signal(s)"} /><Fact label="Team / contact" value={`${candidate.facts.teamSize} · ${candidate.facts.contactChannels.length} path(s)`} /><Fact label="Network" value={candidate.facts.managesCreators || candidate.facts.agencyOrStudio || candidate.facts.creatorEducator ? "Creator network signal" : "No network signal"} /><Fact label="Distribution" value={`${candidate.facts.distributionChannelCount} channel(s)`} /></div> : <p className="mt-4 text-sm leading-6 text-slate-500">This candidate came from the legacy v2 CSV, which supplied component scores without structured facts. Human overrides and totals remain auditable.</p>}</section>
          <section className="panel p-5"><div className="flex items-start justify-between gap-4"><div><p className="eyebrow">Public evidence</p><h3 className="panel-title">Evidence chain</h3></div><Badge tone="amber">Candidate-level evidence</Badge></div><p className="mt-3 text-sm leading-6 text-slate-500">The source CSV provides up to two public evidence entries per candidate. They support the profile as a whole; this MVP does not invent a 15-item evidence mapping.</p><div className="mt-5 space-y-3">{evidence.map((item) => <a key={item.id} href={item.url} target="_blank" rel="noreferrer" className="evidence-card block rounded-xl border border-slate-200 p-4 transition hover:border-teal-300 hover:bg-teal-50/30"><p className="evidence-link text-xs">Source {item.position} ↗</p><p className="mt-1 break-all text-xs text-slate-400">{item.url}</p><p className="mt-2 text-sm leading-6 text-slate-700">{item.summary}</p></a>)}</div></section>
        </div>
        <div className="space-y-6">
          <section className="panel p-5"><p className="eyebrow">Score breakdown</p><div className="mt-4 grid grid-cols-3 gap-2"><div className="score-total"><p className="text-xs text-slate-400">Fit</p><p className="score-total-value">{candidate.fitScore}</p></div><div className="score-total"><p className="text-xs text-slate-400">Activation</p><p className="score-total-value">{candidate.activationScore}</p></div><div className="score-total"><p className="text-xs text-slate-400">Network</p><p className="score-total-value">{candidate.networkScore}</p></div></div><div className="mt-5 space-y-2">{components.map((component) => { const definition = getScoreDefinition(component.scoreKey); const signal = scoreSignals.find((item) => item.key === component.scoreKey); return <button type="button" key={component.scoreKey} onClick={() => selectScore(component.scoreKey)} className={`score-choice-button w-full text-left ${selectedKey === component.scoreKey ? "score-choice-selected" : ""}`}><span className="flex items-center justify-between"><span><span className="block text-sm font-medium text-slate-700">{definition?.label ?? component.scoreKey}</span><span className="mt-0.5 block text-xs text-slate-400">{definition?.category} · {component.source === "HUMAN_OVERRIDE" ? "Human override" : component.source === "FACT_RULES" ? "Local facts rule" : "CSV input"}</span></span><span className="font-semibold text-slate-800">{component.score} <span className="text-xs font-normal text-slate-400">/ {component.maxScore}</span></span></span>{signal ? <span className="mt-2 block text-xs leading-5 text-slate-500">Automatic rule: {signal.reason}</span> : null}</button>; })}</div></section>
