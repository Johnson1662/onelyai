  ...
  return (
    <div className="app-page space-y-6">
      ...
      <form className="space-y-6" onSubmit={submit}>
        ...
        <section className="panel p-5"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="eyebrow">Local score preview</p><h3 className="panel-title">Rules calculate all 15 components</h3><p className="panel-copy">No model call or hidden score input is used. The saved component values can still be overridden by a human with a reason.</p></div><span className="badge badge-teal">facts rules · v2.1</span></div><div className="mt-5 grid gap-3 sm:grid-cols-4"><Total label="Fit" value={score.scores.fit} /><Total label="Activation" value={score.scores.activation} /><Total label="Network" value={score.scores.network} /><Total label="Priority" value={score.scores.priorityScore} tone={scoreTone(score.scores.priorityScore)} /></div><div className="mt-5 divide-y divide-slate-100 rounded-xl border border-slate-200">{score.signals.map((signal) => <div key={signal.key} className="score-choice grid gap-1 text-sm sm:grid-cols-[1fr_70px_2fr] sm:items-center"><span className="font-medium text-slate-700">{signal.category} · {signal.label}</span><span className="font-semibold text-slate-900 sm:text-center">{signal.score}/{signal.max}</span><span className="text-xs leading-5 text-slate-500">{signal.reason}</span></div>)}</div></section>
...
function FactCheckbox({ label, checked, onChange }: { label: string; checked: boolean; onChange: (checked: boolean) => void }) {
  return <label className="fact-option flex items-center gap-2 text-sm text-slate-700"><input type="checkbox" checked={checked} onChange={(event) => onChange(event.target.checked)} /><span>{label}</span></label>;
}
...
function Total({ label, value, tone = "slate" }: { label: string; value: number; tone?: "slate" | "teal" | "amber" | "rose" | "blue" }) {
  ...
  return <div className="score-total"><p className="text-xs text-slate-400">{label}</p><p className={`score-total-value ${colors[tone]}`}>{value}</p></div>;
}
